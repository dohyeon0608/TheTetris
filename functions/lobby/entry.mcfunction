...
execute as @a[x=-55,y=-38,z=7,dx=1,dy=0,dz=2,c=1,tag=join] if score time main matches 0 at @s run tellraw @a {"rawtext":[{"text":"§l§bTETRIS 》 §r§e"},{"selector":"@s"},{"text":"§f님이 게임을 시작하였습니다!"}]}
execute as @a[x=-55,y=-38,z=7,dx=1,dy=0,dz=2,c=1,tag=join] if score time main matches 0 at @s run function game/start
